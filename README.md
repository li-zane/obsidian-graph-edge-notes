...
- Shows optional detail text when you click a label, and closes it when you click anywhere outside the popup.
- Adds a command to create a new relation for the current note.
- Supports a plugin-level default label color and per-relation YAML color overrides.
...
`label` is what appears on the edge. `detail` is optional and appears in the click popup, where the text can be selected and copied. `color` is optional and overrides the plugin's default label color for that relation.
...
Rendered labels are clickable for viewing relation detail. Edit relations through the command palette command **Add graph relation to current note** or by editing frontmatter directly.
